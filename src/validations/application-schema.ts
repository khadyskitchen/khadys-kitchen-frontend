...
// Length caps mirror the backend `applySchema` (application-validation.ts):
// fullName 150, phone 20, location 150, message 2000. The field is named
// `fullName` to match the backend, so its field errors land inline.
export const applicationSchema = z.object({
  ...
  phone: z
    ...
    .trim()
    .min(1, REQUIRED_MESSAGE)
    .max(20, "Please enter a shorter phone number (max 20 characters)."),
  email: z
  ...
  location: z
    .string()
    .trim()
    .max(150, "Please keep your location under 150 characters.")
    .optional(),
